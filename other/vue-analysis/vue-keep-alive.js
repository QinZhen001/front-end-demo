...
// 在点击button时候，coma与comb两个组件会发生切换，但是这时候这两个组件的状态会被缓存起来，比如说coma与comb组件中都有一个input标签，
// 那么input标签中的内容不会因为组件的切换而消失。
...
render()
{
    ...
    const componentOptions: VNodeComponentOptions = vnode && vnode.componenttOptions
    if (componentOptions) {
        // check pattern
        ...
        if (this.cache[key]) {
            vnode.componentInstance = this.cache[key].componentInstancel
        } else {
...
// 检测include与exclude属性匹配的函数很简单，include与exclude属性支持字符串如"a,b,c"这样组件名以逗号隔开的情况以及正则表达式。
// matches通过这两种方式分别检测是否匹配当前组件。
if (this.cache[key]) {
...
// 接下来的事情很简单，根据key在this.cache中查找，如果存在则说明之前已经缓存过了，直接将缓存的vnode的componentInstance（组件实例）覆盖到目前的vnode上面。
// 否则将vnode存储在cache中。
